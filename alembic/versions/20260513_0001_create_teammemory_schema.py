...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint("role IN ('owner', 'manager', 'member', 'viewer')", name="project_members_role_check"),
    ...
    op.create_table(
        ...
        sa.CheckConstraint("status IN ('uploaded', 'processing', 'completed', 'failed')", name="documents_status_check"),
        sa.ForeignKeyConstraint(["project_id"], ["projects.id"], ondelete="CASCADE"),
    ...
    op.create_table(
        ...
        sa.Column("vector_store", sa.String(length=80), nullable=False, server_default="faiss"),
        sa.Column("vector_id", sa.String(length=255), nullable=False),
        sa.Column("embedding_model", sa.String(length=120), nullable=False),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.ForeignKeyConstraint(["chunk_id"], ["document_chunks.id"], ondelete="CASCADE"),
        sa.UniqueConstraint("vector_store", "vector_id", name="uq_chunk_embeddings_vector"),
    )
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.CheckConstraint("status IN ('todo', 'in_progress', 'done', 'delayed', 'cancelled')", name="todos_status_check"),
        sa.CheckConstraint("priority IN ('low', 'medium', 'high', 'urgent')", name="todos_priority_check"),
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.func.now()),
        sa.CheckConstraint("severity IN ('low', 'medium', 'high', 'critical')", name="issues_severity_check"),
        sa.CheckConstraint("status IN ('open', 'in_progress', 'resolved', 'closed')", name="issues_status_check"),
        sa.ForeignKeyConstraint(["project_id"], ["projects.id"], ondelete="CASCADE"),
