...
SYSTEM_PROMPT = (
    ...
    "이슈와 Todo의 연결은 todos[].linked_issue_id가 issues[].id와 일치할 때만 성립한다. "
    "그 외의 인과관계를 추측하지 마라. "
    "대괄호로 감싼 형식 설명이나 자리표시자를 결과에 그대로 출력하지 마라. 실제 값만 써라."
)
...
- 묶는 기준은 오직 데이터다. 각 todo의 linked_issue_id가 어떤 issue의 id와 같을 때만 그 이슈 아래에 적어라.
  linked_issue_id가 null인 todo는 어떤 이슈에도 매달지 말고 마지막 "기타 진행 업무"로 모아라.
  두 항목이 같은 고객사나 품목을 다룬다는 이유만으로 인과관계를 추측해 묶지 마라.
- 각 이슈 항목은 이슈 제목, 담당자(assignee), 원인(risk_reason), 연결된 업무, 근거 문서(source) 순으로 쓴다.
  값은 전부 grounding JSON에서 그대로 가져와라. 형식을 설명하는 대괄호 라벨이나 자리표시자를
  결과에 그대로 출력하지 마라. 결과에는 실제 값만 있어야 한다.
- 연결된 todo가 하나도 없는 이슈는 "직접 연결된 후속 업무: 없음 (후임자 판단 필요)"라고 적어라.
  연결 Todo가 없다는 이유로 이슈 자체를 빼는 것은 인수인계 누락이므로 금지한다.
- grounding의 모든 이슈는 빠짐없이 §3에 남겨라. 이슈 제목 자체를 업무처럼 재서술하지 마라.
- §3에 등장하는 업무는 전부 todos 배열에 실제로 있는 title이어야 한다. 새 업무를 만들어내지 마라.
...
class HandoverDraftService:
    ...
    async def generate(self, handover_input: dict[str, Any], *, document_type: str = "handoff") -> str | None:
        ...
        try:
        ...
        except (AzureOpenAIConfigError, ModuleNotFoundError, RuntimeError, ValueError) as exc:
            logger.error(
                "handover draft LLM call failed (document_type=%s): %s: %s",
                document_type,
                type(exc).__name__,
                exc,
            )
            ...
            logger.exception(
                "handover draft LLM call raised unexpected error (document_type=%s): %s",
                document_type,
                type(exc).__name__,
            )
            return None

        if not self._is_complete(content, document_type=document_type):
            logger.error(
                "handover draft rejected as incomplete (document_type=%s, length=%d): %r",
                document_type,
                len(content),
                content[:200],
            )
