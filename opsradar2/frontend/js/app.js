...
function showAnalysisResult(fname) {
  ...
  const data = {
    meeting:{t:5,i:2,b:1,cm:'회의록 · 1페이지 · chunk #1',cc:'운영 회의. 운영 API 응답 지연 <span class="chunk-hl">5초 초과</span>. <span class="chunk-hl">담당자 A 추가 시간 필요</span>.',reason:'회의록에서 지연·초과·필요 키워드 반복 감지'},
    chat:{t:3,i:2,b:1,cm:'채팅 로그 · chunk #6',cc:'담당자 A: <span class="chunk-hl">운영 API 타임아웃</span>이 계속 발생합니다. <span class="chunk-hl">재시도 로직</span> 필요.',reason:'채팅 로그에서 API 타임아웃 반복 언급 감지'},
    issue:{t:2,i:3,b:1,cm:'이슈 로그 · chunk #8',cc:'운영 API 응답 지연 <span class="chunk-hl">5초 초과</span>. 담당: <span class="chunk-hl">담당자 A</span>. <span class="chunk-hl">재시도 로직 추가 필요</span>.',reason:'이슈 로그에서 5초 초과 지연 및 담당자 미조치 패턴 감지'}
  };
...
function getFilteredTodos(){
  ...
  return todos.filter(t=>t.status==='done'||t.status==='rejected');
}
...
function renderTodoDetail(id){
  ...
  <div style="padding:12px 14px;border-top:1px solid var(--border);display:flex;gap:5px">
    ...
    t.status==='approved'?`<div class="tbtn primary" style="flex:1;justify-content:center;background:var(--success);border-color:var(--success)" onclick="doneTodo(${t.id})">완료</div><div class="tbtn" style="flex:1;justify-content:center" onclick="undoTodo(${t.id})">↩ 되돌리기</div>`:
...
function updateTodoCounts(){
  ...
  const dn=todos.filter(t=>t.status==='done'||t.status==='rejected').length;
  ...
  ['t-ai-cnt','t-in-cnt','t-done-cnt'].forEach((id,i)=>{const el=document.getElementById(id);el.className=i===0&&ai>0?'badge b-warn':'badge b-gray';});
}
function switchTodoTab(tab){G.currentTodoTab=tab;G.selectedTodoId=null;document.getElementById('todoDetailEmpty').style.display='flex';document.getElementById('todoDetailContent').style.display='none';document.querySelectorAll('#s-todo .tab').forEach((el,i)=>{el.classList.toggle('active',['ai','inprogress','done'][i]===tab);});document.getElementById('todoAINotice').style.display=tab==='ai'?'flex':'none';renderTodos();}
function openEditModal(id){G.editTargetId=id;const t=todos.find(x=>x.id===id);document.getElementById('editTitle').value=t.title;document.getElementById('editAssignee').value=t.assignee||'담당자 A';document.getElementById('editModal').classList.add('show');}
function saveEdit(){const t=todos.find(x=>x.id===G.editTargetId);t.title=document.getElementById('editTitle').value.trim();t.assignee=document.getElementById('editAssignee').value;closeModal('editModal');renderTodos();if(G.selectedTodoId===G.editTargetId)renderTodoDetail(G.editTargetId);showToast('수정되었습니다.','info');}
...
function renderIssueDetail(id){
  ...
  dc.innerHTML=`
    ...
    ${hasTodo?`<div class="detail-section">생성된 대응 Todo</div>${G.createdTodosFromIssue.filter(t=>t.issueId===id).map(t=>`<div style="background:var(--success-soft);border-radius:var(--radius-sm);padding:8px 10px;font-size:11px;color:var(--success);display:flex;align-items:center;gap:6px;margin-bottom:4px"><i class="ti ti-checkbox" style="font-size:12px"></i>${t.title}</div>`).join('')}`:''}`;
  ...
  setTimeout(async ()=>{
    try{
      ...
      if(apiIssueId){
        ...
        todos.unshift({id:Date.now()+1,title,src:`이슈 #${G.createIssueId}`,srcChunk:'이슈 연결',assignee,priority,confidence:null,status:'pending',type:'manual',chunk:null,grounds:['이슈 기반 생성'],risk:''});
      }
...
function getInitialChatHtml(){
  return `
    ...
    <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:8px">
      ...
      <div class="sq" onclick="sendMsg('담당자 A 5/26 부재')">담당자 A 5/26 부재</div>
      <div class="sq" onclick="sendMsg('담당자 A 다음주 화요일 외부 일정')">담당자 A 다음주 화요일 외부 일정</div>
    </div></div></div>`;
}
...
async function sendMsg(text){
  ...
  try{
    ...
    removeTyping();
    appendChatMsg('ai',data.answer||'AI 응답이 비어 있습니다.',null);
  }catch(error){
    removeTyping();
...
function inferChatContext(text, src){
  const docs = src ? [src] : (/위험|이슈|Risk|리스크|장애|API|DB/i.test(text) ? ['운영 로그 분석 결과', '이슈 관리 현황'] : ['운영 데이터 인덱스']);
...
function appendChatMsg(role,text,src=null,withBtn=false){
  ...
  if(role==='user'){
    ...
    div.innerHTML=`<div class="msg-av av-ai"><i class="ti ti-sparkles" style="font-size:13px"></i></div><div><div class="ai-analysis-card"><div class="ai-card-kicker">AI OPERATION ANALYSIS</div><div class="ai-card-title">운영 데이터 기반 분석 결과</div><p class="text-content">${escapeHtml(text)}</p>${src?`<div class="ai-card-source"><i class="ti ti-file-text" style="font-size:11px"></i> 출처: ${escapeHtml(src)}</div>`:''}</div>${withBtn?'<div style="margin-top:6px"><div class="tbtn" style="font-size:10px;padding:4px 10px;color:var(--accent)" onclick="nav(\'knowledge\')"><i class="ti ti-transfer"></i> 인수인계 문서 생성</div></div>':''}</div>`;
  ...
  return `
    ...
    <div style="display:flex;flex-direction:column;gap:6px">
      <div style="background:var(--warn-soft);border:1px solid rgba(196,124,0,.15);border-radius:var(--radius-sm);padding:8px 10px;font-size:11px">
        <div style="font-weight:500;color:var(--text);display:flex;align-items:center;gap:5px;margin-bottom:3px">담당자 A 업무 대리 <span class="badge b-warn" style="font-size:9px">진행중</span></div>
  ...
  return `
    ...
    <div class="kcard">
      ...
      <div class="kcard-body">
        ...
        <div class="kb-next"><i class="ti ti-arrow-right" style="font-size:13px;flex-shrink:0"></i><div><strong>내가 할 것:</strong> 재시도 로직 테스트 → 결과 팀 공유 → 이슈 상태 업데이트. 긴급 시 담당자 A에게 연락.</div></div>
    ...
    <div class="kcard">
      ...
      <div class="kcard-body">
        <div class="kb-next"><i class="ti ti-arrow-right" style="font-size:13px;flex-shrink:0"></i><div><strong>복귀 전 인계:</strong> 내가 처리한 내용을 기록해두면 담당자 A 복귀 시 AI가 자동으로 브리핑 문서를 생성합니다.</div></div>
        ...
        <div class="checklist-item"><div class="cl-box"></div><div class="cl-text" style="flex:1">미처리 항목 담당자 A 복귀 시 전달 예약</div></div>
...
function kCtxOffboard() {
  return `
    ...
    <div style="background:var(--surface2);border-radius:var(--radius-sm);padding:8px 10px;font-size:11px;margin-bottom:12px">
      <div style="font-weight:500;color:var(--text)">담당자 A</div>
  ...
  const map = {
    ...
    emergency: {
      ...
      sections:[
        ...
        ['담당자','PM 관리자, Backend 담당자 A, Infra 담당자 B가 1차 대응 담당입니다.'],
