    ...
    <section className={`hc-step-body${step === 3 ? " preview-wide" : ""}`}>
      {step === 1 && <div className="hc-condition-panel"><header><h2>생성 조건</h2><p>업무 기록을 찾을 범위와 결과 문서에 포함할 항목을 선택하세요.</p></header>
        <div className="hc-form-grid">{onboarding ? <>
          <Field label="온보딩 대상" value={conditions.target} options={members} onChange={(v) => onCondition("target", v)} /><Field label="소속 팀" value={conditions.department} options={DEPARTMENTS} onChange={(v) => onCondition("department", v)} /><Field label="조회 기간" value={conditions.period} options={PERIODS} onChange={(v) => onCondition("period", v)} />
        </> : <>
      ...
      {step === 3 && (generating
        ? <div className="hc-generating"><span className="hc-spinner" aria-hidden="true" /><p>AI가 인수인계서를 생성 중입니다…</p></div>
        : <>
